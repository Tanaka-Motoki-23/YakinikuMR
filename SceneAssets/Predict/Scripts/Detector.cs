...
public class Detector : MonoBehaviour
{
    ...
    // パラメータ
    public int IMAGE_SIZE = 416; // 画像サイズ
    ...
    [SerializeField]
    private string OUTPUT_NAME = "reshape";

    // 出力のパース
    [SerializeField]
    private int CLASS_COUNT = 2; // クラス数
    [SerializeField]
    private int ROW_COUNT = 13; // 行
    [SerializeField]
    private int COL_COUNT = 13; // 列
    ...
    public IEnumerator Predict(Color32[] picture, System.Action<IList<BoundingBox>> callback)
    {
        ...
        using (var tensor = TransformInput(picture, IMAGE_SIZE, IMAGE_SIZE))
        {
            ...
            var new_shape = new[] { 1, 1, 1, 1, 1, ROW_COUNT, COL_COUNT, (BOX_INFO_FEATURE_COUNT + CLASS_COUNT) * BOXES_PER_CELL };
            output = TensorExtensions.Reshape(output, new_shape);
